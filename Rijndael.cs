...
namespace BootstrapService
{
    public class Rijndael
    {
        ...
        public static string DecryptV2(string cipherText, string randomKeyText, byte[] randomKeyAsByteArray)
        {
            // Convert strings defining encryption key characteristics into byte
            // arrays. Let us assume that strings only contain ASCII codes.
            // If strings include Unicode characters, use Unicode, UTF7, or UTF8
            ...
            // Generate decryptor from existing key bytes and initialization vector. 
            // Key size will be defined based on the number of the key bytes
            byte[] keyBytes = Encoding.ASCII.GetBytes(randomKeyText);
            keyBytes = randomKeyAsByteArray;
